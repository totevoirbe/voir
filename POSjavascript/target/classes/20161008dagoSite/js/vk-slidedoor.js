 ...
 * 
 * Copyright 2012-2014, Van Khuong (www.sothichweb.com) Licensed under the MIT
 * Released on: Nov 21, 2012
 * 
 * Tut:
 * http://sothichweb.com/article/xay-dung-plugin-slide-door-voi-jquery/d967abd
 * Demo:
 * http://sothichweb.com/Demos/slidedoor/Demo-xay-dung-plugin-slidedoor-voi-jquery.html
 * 
...
(function($) {
	...
	$.fn.vkSlidedoor = function(cfg) {
		...
		var ops = {
			...
			trigger : 'dt',
			autoplay : false,
			direction : 'ltr', // or 'rtl'(right to left), direction
			// slide-door, apply for the mode autoplay is
			// true
			looptimeout : 3000, // set timeout for slide-door, apply for the
			// mode autoplay is true
			dtwidth : 29,
			ddpadleft : 1,
			ddwidth : 392
		};
		...
		return this.each(function() {
			var $this = $(this), $dls = $this.find(ops.dl), $dts = $this
					.find(ops.trigger), $ele = $this.find(ops.wrapper);
			...
			var overwidth = $dls.length * ops.dtwidth + ops.ddwidth
					- ops.ddpadleft, mainwidth = overwidth + ops.ddwidth + 10;
			...
			$this.find(ops.dl + '.current').nextAll(ops.dl).elemsMove(
					ops.ddwidth);
			...
			var indexthis = getParameter("initPage");
			$ele.activeDoors(ops, indexthis);

			$(".selectBook").click(function(event) {
				event.preventDefault();
				if (ops.autoplay) {
					clearInterval(timeout);
				}

				var paramNextPage = this.href.split("=");

				$ele.activeDoors(ops, paramNextPage[1]);
	...
	$.fn.activeDoors = function(ops, indexthis) {
		...
		var $curr = this.find('.current').removeClass('current'), indexcurr = $curr
				.index(), $dls = this.find(ops.dl), dlslen = $dls.length;

		// animation with current element and this element with click event
		// context
		if (arguments.length == 2) {
			...
			if (indexcurr == (dlslen - 1)) {
				this.find(ops.dl + ':first-child').addClass('current').nextAll(
						ops.dl).elemsMove(ops.ddwidth);
			} else {
	...
	$.fn.elemsMove = function(val) {
		return this.each(function() {
			var leftcurr = $(this).css('left'), newcurr = parseInt(leftcurr)
					+ val;
